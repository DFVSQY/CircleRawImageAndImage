...
public class CircleRawImageEditor : RawImageEditor
{
	...
	public override void OnInspectorGUI()
	{
		...
		// 不同填充模式限制不同最小段数
		CircleRawImage.FillMode mode = (CircleRawImage.FillMode)fillModeProp.enumValueIndex;
		int minSegment = mode == CircleRawImage.FillMode.FillOutside ? 4 : 3;
		segmentProp.intValue = Mathf.Max(minSegment, segmentProp.intValue);

		// 根据 mode 的值决定是否显示 edgeThickness 属性
		if (mode == CircleRawImage.FillMode.Edge)
		{
